...
public class BoardDAO {
   ...
   public BoardVo updateGetArticle(int num) {
	...
	//검색한 내용이 몇개 있는지를 반환하는 기능(what: 검색조건, conttent:검색내용)
	...
	public int getArticleCount(String searchWhat,String searchText) {
	      ...
	      try {
	         ...
	         conn = ConnUtil.getConnection();
	         pstmt = conn.prepareStatement("select count(*) from board where "+searchWhat+" like '%"+searchText+"%'");
	         rs = pstmt.executeQuery();
	...
	public List<BoardVo> getArticles(String searchWhat,String searchText, int start, int end){
	      ...
	      try {
	         ...
	         pstmt = conn.prepareStatement("select * from("
	                 + "select rownum rnum, num, writer, email, subject, "
	                 + "pass, regdate, readcount, ref, step, depth, content, ip from("
	                 + "select * from board where "+searchWhat+" like '%"+searchText+"%' order by num desc, step asc))"
	                    + "where rnum >= ? and rnum <= ?");
	         pstmt.setInt(1, start);
	         pstmt.setInt(2, end);
	         rs = pstmt.executeQuery();
