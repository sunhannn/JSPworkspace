...
<%@ include file="/header.jsp" %>
	<div class="content_all">
		...
		<section id="pageList">
			...
			%>
			<button type="button" class="paging_btn" onClick="location.href='memberSearch.go?search=<%=list_search%>&search_value<%=list_search_value%>'">이전</button>&nbsp;
			<%
			}
			%>
			...
			%>
			<button class="paging_btn" onClick="location.href='memberSearch.go?search=<%=list_search%>&search_value<%=list_search_value%>?page=<%=a%>'"><%=a%>
			</button>
			<%
			}
			%>
			...
			%>
			<button type="button" class="paging_btn" onClick="location.href='memberSearch.go?search=<%=list_search%>&search_value<%=list_search_value%>?page=<%=nowPage + 1%>'">다음
			</button>
			<%
			}
			%>
		...
		%>
		<section id="emptyArea">검색된 회원이 없습니다.</section>
		<%
		}
		%>
